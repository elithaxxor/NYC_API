	...
	func fetchGrades() {
		...
		URLSession.shared.dataTaskPublisher(for: url)
			...
			.tryMap { (data, response) in
				...
				print("[simple-grades] found data \(data.description)")
				print(" \(data)")
...
class sampleAPI {
	...
	func getPublisher() -> AnyPublisher<String, URLError> {
		URLSession.shared.dataTaskPublisher(for: url)
	...
	// Use this function to subscribe from @Escaping calls
	func getFuturePublisher() -> Future<String, Error> {
		Future { promise in
			self.downloadEscapingClosure { val, error in
				if let error = error {
	...
	// MARK: URLSessions / Escaping
	/// Use this To download data when combine is not pressent
	func downloadEscapingClosure(completionHandler: @escaping (_ value: String, _ error: Error? ) -> ()) {
		URLSession.shared.dataTask(with: url) { data, response, error  in
			completionHandler("ne value 2", nil)
		}
		.resume()
	}
	
		// MARK: TO USE WITH ESCAPING CLOSURES (non Combine data)
		func getDataFromEscaping() {
			downloadEscapingClosure { [weak self] value, error in
				self?.title = value
			}
		}
